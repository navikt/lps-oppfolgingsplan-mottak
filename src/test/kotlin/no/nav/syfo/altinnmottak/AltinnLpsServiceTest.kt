...
class AltinnLpsServiceTest : DescribeSpec({
    ...
    beforeTest {
        ...
        coEvery { isdialogmeldingConsumer.sendLpsPlanToFastlege(any(), pdfByteArray) } returns true
    ...
    describe("Receive Altinn-LPS from altinnkanal-2") {
        ...
        it("Receive LPS with BistandFraNAV and DelMedFastlege set to true") {
            ...
            coVerify(exactly = 1) {
                isdialogmeldingConsumer.sendLpsPlanToFastlege(arbeidstakerFnr, pdfByteArray)
            }
            verify(exactly = 1) {
        ...
        it("Receive LPS with BistandFraNAV and DelMedFastlege set to false") {
            ...
            coVerify(exactly = 0) {
                isdialogmeldingConsumer.sendLpsPlanToFastlege(arbeidstakerFnr2, pdfByteArray)
            }
            verify(exactly = 0) {
                navLpsProducer.sendAltinnLpsToNav(any())
            }
