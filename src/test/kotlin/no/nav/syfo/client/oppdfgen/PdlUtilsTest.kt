    ...
    DescribeSpec({
        ...
        describe("Validation of adress string") {
            ...
            it("Should set correct spaces in address string if all fields are present") {
                coEvery { pdlClient.getPersonInfo(any()) } returns
                    PdlHentPerson(
                        PdlPerson(
                            adressebeskyttelse =
                                listOf(
        ...
        describe("getPersonInfoWithRetry") {
            ...
            it("Should throw PdlServerException after exhausting all retries") {
                val fnr = "12345678901"
                coEvery { pdlClient.getPersonInfo(fnr) } throws PdlServerException("Server error")

                shouldThrow<PdlServerException> {
                    pdlUtils.getPersonInfoWithRetry(fnr, 3)
                }
