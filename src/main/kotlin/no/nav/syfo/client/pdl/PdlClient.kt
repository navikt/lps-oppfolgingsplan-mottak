...
class PdlClient(
    ...
    private suspend fun postCallToPdl(requestBody: PdlRequestInterface): HttpResponse? {
        val token =
            azureAdClient.getSystemToken(urls.pdlScope)?.accessToken
    ...
    private suspend fun handleSokAdresseResponse(
    ...
    private fun handlePdlErrors(
        ...
        when (error.extensions.code) {
            ...
            "bad_request" -> throw PdlBadRequestException(
                "Could not $operation: 'Ugyldig ident/Ugyldig spørring/For stor spørring': $errorMessage",
            )
