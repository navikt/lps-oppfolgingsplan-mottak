...
fun getPropertyFromSecretsFile(name: String) = File("$SERVICE_USER_MOUNT_PATH/$name").readText()

fun getLocalEnv(): Environment = objectMapper.readValue(File(LOCAL_PROPERTIES_PATH), Environment::class.java)

fun Environment.isDev(): Boolean = this.application.cluster == DEV_CLUSTER
